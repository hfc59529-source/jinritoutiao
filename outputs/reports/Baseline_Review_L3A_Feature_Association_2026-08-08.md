...
**观察**：题材之间字数均值最高与最低相差约518字（宏观经济/行业数据1,222.6 vs 社会民生/案件704.8）；结构类型之间相差约230字（解读/分析1,123.4 vs 事件通报/新闻转述893.1）。这是描述性的组间均值/中位数差异（Descriptive Group Difference），尚未检验组内方差、组间差异相对组内波动的大小，也未做任何关联检验——而且从min–max范围看，各组字数区间存在大量重叠（例如题材维度：宏观经济/行业数据539–1,815、科技/国际563–2,139、企业/商业动态543–2,003、消费/维权562–1,766，彼此高度重叠；结构维度：解读/分析327–2,139与事件通报/新闻转述524–2,036同样大量重叠）。在46篇、6个题材类别的小样本条件下，均值差异不足以确认稳定关联，本层不引入正式统计检验（避免在小样本下制造虚假的显著性精确感）。

当前样本中，解读/分析类的平均和中位字数高于事件通报/新闻转述类；宏观经济/行业数据、科技/国际、企业/商业动态三个题材的平均字数高于职场/收入、社会民生/案件。这些是本层观察到的事实，不代表稳定关系。

**Proxy Hypothesis（待检验假设，非结论）**：结合L2观察（高展现组、高阅读率组、收益组字数均值均高于对照组）与本层观察（不同题材/结构之间字数分布本身存在差异），可以提出：

> **H-L3-001**：L2中观察到的 word_count 与表现（展现/阅读率/收益）之间的关系，可能至少部分受到 topic_category / structural_form 的混杂。

这只是一个假设，尚未获得本层证据的确认或排除，只能交给 L3-B 通过控制 topic_category / structural_form 后检验 word_count 差异是否仍然存在来判断。
...
- **word_count ↔ topic_category**：不同题材之间字数均值/中位数存在差异（最高与最低相差约518字），但各组 min–max 区间大量重叠，未做正式关联检验，n拆分到6组后部分组样本量偏小，证据等级为候选而非确认。
- **word_count ↔ structural_form**：不同结构类型之间字数均值/中位数存在差异（相差约230字），同样存在大范围重叠，证据等级为候选。
- **H-L3-001（Proxy Hypothesis，待检验）**：L2观察到的 word_count–表现关系，可能至少部分受到 topic_category / structural_form 的混杂——这是一个待 L3-B 检验的假设，本层未获得确认或排除。
...
### 无法判断（Discriminative Power 不足，非"特征无效"）
- **stakes_has_specific_amount_or_quantity**：FALSE组仅1篇，Small-N，无法判断。
- **stakes_has_action_advice**、**stakes_has_specific_amount_or_quantity**：这两个特征在当前46篇样本中缺乏足够方差（46:0 与 45:1），因此没有足够的 Discriminative Power（区分能力）用于本轮关联检验。这不等于证明特征本身无效，只说明当前定义无法在这批样本之间制造足够的方差来做比较；是否需要调整 Schema，留待整轮 Baseline Review 完成后再评估，本层不修改 Schema。
